...
parser.add_argument("-t", "--threshold", required=False,
                    help="Threshold used for Artifact detection. default is 55 and the normal range is (40 90)")
parser.add_argument("-g", "--gap", required=False,
                    help="The number of samples ignored around R peaks. default is 10 and the normal range is ("
                         "10 100)")
parser.add_argument("-m", "--margin", required=False, help="the number of R peaks before and after an artifact that "
                                                           "will be included in the episodes. default is 4 and the "
                                                           "normal range is (1 7)")
...
g = int(str(10) or args["gap"])
output_directory = args["output"] or output_directory
t = int(args["threshold"] or '55')
m = int(args["margin"] or '4')
